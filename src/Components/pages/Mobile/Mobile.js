...
const Mobile = () => {
  ...
  const handleDeleteProduct = async (product) => {
    try {
      ...
      const { data: response } = await axiosInstance.delete(`mobiles/${product._uuid}`);
      setSuccessAPI(true);
      console.log(response.items);
    } catch (error) {
      console.error(error.message);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      if (dataDetail) {
        try {
          const { data: response } = await axiosInstance.put('mobiles', [
            {
              ...dataDetail,
        ...
        try {
          const { data: response } = await axiosInstance.post('mobiles', [
            {
              imageProduct: values.productImageLink,
              nameLaptop: [values.productName],
              price: values.price,
