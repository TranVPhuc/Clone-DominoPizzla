...
const Console = () => {
  ...
  const handleDeleteProduct = async (product) => {
    try {
      ...
      const { data: response } = await axiosInstance.delete(`consoles/${product._uuid}`);
      setSuccessAPI(true);
      console.log(response.items);
    } catch (error) {
      console.error(error.message);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      if (dataDetail) {
        try {
          const { data: response } = await axiosInstance.put('consoles', [
            {
              ...dataDetail,
        ...
        try {
          const { data: response } = await axiosInstance.post('consoles', [
            {
              imageProduct: values.productImageLink,
              nameLaptop: [values.productName],
              price: values.price,
