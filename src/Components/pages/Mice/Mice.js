...
import axiosInstance from './configMouseAPI';
...
const Mice = () => {
  ...
  const handleDeleteProduct = async (product) => {
    try {
      ...
      const { data: response } = await axiosInstance.delete(`mices/${product._uuid}`);
      setSuccessAPI(true);
      console.log(response.items);
    } catch (error) {
      console.error(error.message);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      if (dataDetail) {
        try {
          const { data: response } = await axiosInstance.put('mices', [
            {
              ...dataDetail,
        ...
        try {
          const { data: response } = await axiosInstance.post('mices', [
            {
              imageProduct: values.productImageLink,
              nameLaptop: [values.productName],
              price: values.price,
